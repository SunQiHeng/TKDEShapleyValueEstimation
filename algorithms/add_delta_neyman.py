...
def add_delta_neyman_shap(dynamic_game, initial_m, local_m, proc_num=1) -> np.ndarray:
    ...
    for j in range(math.ceil(n / 2) - 1, n):
        ...
        var_sum += np.sqrt(sigma_dcc_j[j] + sigma_dcc_n_j[j] + sigma_cc_j[j] + sigma_cc_n_j[j])

    local_m -= 2*count
    m = np.zeros(n)
    if var_sum != 0:
        for j in range(math.ceil(n / 2) - 1, n):
            m[j] = max(0, math.ceil(local_m/2* np.sqrt(sigma_dcc_j[j] + sigma_dcc_n_j[j] + sigma_cc_j[j] + sigma_cc_n_j[j]) / var_sum))
    else:
        for j in range(math.ceil(n / 2) - 1, n):
    ...
    pool = Pool()
    func = partial(_add_delta_neyman_task, dynamic_game)
...
def _add_delta_neyman_task(dynamic_game,m) -> np.array:
    """
