...
def add_delta_shap(dynamic_game, m, proc_num=1) -> np.ndarray:
    ...
    pool = Pool()
    func = partial(_add_delta_task, dynamic_game)
...
def _add_delta_task(dynamic_game,local_m) -> np.array:
    """
    ...
    for _ in trange(int(local_m/2)):
        ...
        add_index = np.array([dynamic_game.n-dynamic_game.dynamic_size+i for i in range(dynamic_game.dynamic_size)])

        u_original_left = dynamic_game.eval_utility([x for x in idxs[:j] if x not in add_index])
        ...
        temp = np.zeros(n)
        temp[idxs[j:]] = 1
        complementary_contributions[n - j, :] += temp * (u_right - u_left)
        count[n - j, :] += temp
        complementary_contributions_difference[n-j,:] += temp*[(u_right-u_left)-(u_original_right-u_original_left)]
