...
for i in range(0,6):
    ...
    pre_sv = ori_sv
    for j in range(10):
        dynashap = DeltaShap(X_train[:490+j],Y_train[:490+j],X_test,Y_test, model, pre_sv)
        temp_sv = dynashap.add_single_point(X_train[490+j],Y_train[490+j], m=int(args.samples/args.new_players/2/10))
        pre_sv = temp_sv
        McDelta = temp_sv
    temp = []
...
write_to_csv(results, './exp_results/bank_svm_add_McDelta.csv')
